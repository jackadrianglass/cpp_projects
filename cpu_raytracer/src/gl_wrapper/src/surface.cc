...
auto Surface::setVertices(gsl::span<const gl::MeshPoint> vertices) -> void
{
  m_vao.bind();
...
auto Surface::setIndices(gsl::span<const glm::ivec3> indices) -> void
{
  m_idxCount = indices.size() * 3;
  m_ibo.uploadIndices(indices, GL_STATIC_DRAW);
}
