...
void printToken(TokenType token, const char *tokenString, int lineno)
{
  ...
  case VOID:
    fprintf(listing,
            "%d: reserved word: %s", lineno, tokenString);
    break;
  case ASSIGN:
    fprintf(listing, "%d: special symbol: =", lineno);
    break;
  case LT:
    fprintf(listing, "%d: special symbol: <", lineno);
    break;
  ...
  case LPAREN:
    ...
    break;
  case RPAREN:
    fprintf(listing, "%d: special symbl: )", lineno);
    break;
  case LBRACK:
    fprintf(listing, "%d: special symbol: [", lineno);
    break;
  ...
  case OVER:
    ...
    break;
  case ENDFILE:
    fprintf(listing, "%d: special symbol: EOF", lineno);
    break;
  case NUM:
    fprintf(listing,
            "%d: NUM, val= %s", lineno, tokenString);
    break;
  case ID:
    fprintf(listing,
            "%d: ID, name= %s", lineno, tokenString);
    break;
  case ERROR:
    fprintf(listing,
            "%d: ERROR: %s", lineno, tokenString);
    break;
  default: /* should never happen */
    fprintf(listing, "%d: Unknown token: %d", lineno, token);
  }
}
  ...
  while (tree != NULL) /* try not to visit null tree children */
  {
    ...
    else if (tree->nodekind == ExpK)
    {
      ...
      case OpK:
        ...
        printToken(tree->attr.op, "", tree->lineno);
        fprintf(listing, "]\n");
        break;
      case IdK:
